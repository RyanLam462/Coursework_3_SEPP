...
public class UserController extends Controller {
        ...
        /**
         * Path to the pre-registered users data file.
         * In this mock implementation the data is hardcoded,
         * but the field is kept for consistency with the
         * class diagram.
         */
        public static final String PREREGISTERED_USERS_FILE_PATH = "data/preregistered_users.csv";

        /**
         * Path to the pre-registered admin staff data file.
         */
        ...
        /**
         * The verification service used to check whether an
         * entertainment provider is legitimate upon
         * registration.
         */
         ...
         *
         * <p>
         * Used internally to register pre-registered users
         * and newly registered entertainment providers.
         * </p>
         *
         ...
         * @return {@code true} if the email contains '@'
         *         and a domain portion
         */
         ...
         * testing purposes, representative accounts are
         * hardcoded here.
         * </p>
         */
        public void addPreregisteredUsers() {
                ...
                users.add(new Student(
                                "student1@ed.ac.uk", "password1",
                                "Alice Smith", 447123456));
                users.add(new Student(
                                "student2@ed.ac.uk", "password2",
                                "Bob Jones", 447654321));
                users.add(new Student(
                                "student3@ed.ac.uk", "password3",
                                "Charlie Brown", 447111222));

                // Pre-registered admin staff
                users.add(new AdminStaff(
                                "admin1@ed.ac.uk", "adminPass1",
                                "Admin Alice"));
                users.add(new AdminStaff(
                                "admin2@ed.ac.uk", "adminPass2",
                                "Admin Bob"));
        }
        ...
        public void registerEntertainmentProvider() {
                ...
                if (!checkCurrentUserIsGuest()) {
                        view.displayError(
                                        "You must log out before registering "
                                                        + "a new account.");
                        return;
                }
                ...
                String businessNumber = view.getInput(
                                "Enter business registration number: ");
                if (businessNumber == null
                                || businessNumber.isBlank()) {
                        view.displayError(
                ...
                if (EPAccountAlreadyExists(
                                email, orgName, businessNumber)) {
                        view.displayError(
                                        "An entertainment provider with the "
                                                        + "same email, organisation name, or "
                                                        + "business number already exists.");
                        return;
                }

                // Verify through external verification service
                boolean verified = verificationService
                                .verifyEntertainmentProvider(
                                                businessNumber);
                if (!verified) {
                        view.displayError(
                                        "Verification failed. The business "
                                                        + "registration number is not valid.");
                        return;
                }

                // Create and register the new EP
                String epName = view.getInput(
                ...
                EntertainmentProvider newEP = new EntertainmentProvider(
                                ...
                                businessNumber, epName, description);
                users.add(newEP);

                // Automatically log in the new EP
                currentUser = newEP;
                view.displaySuccess(
                                "Entertainment provider registered "
                                                + "successfully. Welcome, " + orgName + "!");
        }
                ...
                for (User user : users) {
                        if (user instanceof EntertainmentProvider ep) {
                                if (ep.getEmail()
                                                ...
                                                || ep.getOrgName()
                                                                .equalsIgnoreCase(orgName)
                                                || ep.getBusinessRegistrationNumber()
         ...
         *
         * <p>
         * Prompts the student to set their preferred event
         * type, maximum ticket budget, and whether to show
         * only available events. All inputs are validated
        ...
        public void editPreferences() {
                ...
                if (!checkCurrentUserIsStudent()) {
                        view.displayError(
                                        "Only students can edit preferences.");
                        return;
                }
                ...
                // Prompt for new preferences as comma-separated
                // string of event types
                view.displayInfo(
                                "Enter preferred event types as a "
                                                + "comma-separated list.");
                view.displayInfo(
                                "Valid types: Music, Theatre, Dance, "
                                                + "Movie, Sports");
                view.displayInfo(
                                "Leave blank to clear all preferences.");

                String rawInput = view.getInput(
                                "Enter preferences: ");
                ...
                if (!success) {
                        view.displayError(
                                        "Some preference tokens were not "
                                                        + "recognised. Valid tokens: Music, "
                                                        + "Theatre, Dance, Movie, Sports.");
                }
         ...
         */
        public EntertainmentProvider getEntertainmentProviderOwningEvent(
                        long eventNumber) {
                for (User user : users) {
                        if (user instanceof EntertainmentProvider ep) {
